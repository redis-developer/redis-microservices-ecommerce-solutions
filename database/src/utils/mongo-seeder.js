    ...
    async seedCollections() {
        try {
            if (this.allCollectionsPath) {
                ...
                fs.readdirSync(this.allCollectionsPath).forEach(async (serialFolderName) => {

                    const serialFolderPath = this.allCollectionsPath + "/" + serialFolderName;
                    ...
                    console.log(`Reading ${serialCollectionNames.join(",")} collection(s) data from disk`);
                    let seedData = this.seederLib.readCollectionsFromPath(path.resolve(serialFolderPath), collectionReadingOptions);

                    console.log(`Seeding ${serialCollectionNames.join(",")} collection(s) data to mongodb`);
