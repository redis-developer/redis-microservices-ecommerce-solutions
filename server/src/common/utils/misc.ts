...
const handleProcessAndAppErrors = (_app, _process) => {
    ...
    _process.on('SIGINT', async () => { // interrupt signal like ctrl + c 
    ...
    _process.on('SIGTERM', async () => { //on kill (not supported on windows) 
    ...
    _process.on('exit', async (code) => { //nothing in event loop
